...
const EmotionKeywordRulesPanel:
  ...
  const handleTestAndPreview = (): void => {
    ...
    setStatusMessage(
      `Matched voice keyword "${match.rule.phrase}" -> ${
        getEmotionOptionByKey(
  ...
  return (
    <div className="grid gap-3 rounded-xl border border-white bg-black p-3 text-xs text-white">
      ...
      <div className={panelClass}>
        <div className="mb-2 font-bold text-white">
          Saved voice keyword rules
        </div>

        {rules.length === 0 ? (
          <div className="text-zinc-300">
            No voice keyword rules yet.
          </div>
        ) : (
